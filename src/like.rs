...
pub fn delete_all_likes(_tweet_id: Uuid, conn: &mut DBPooledConnection) -> Result<(), Error> {
    use crate::schema::likes::dsl::*;
    ...
    let res = diesel::delete(likes.filter(tweet_id.eq(_tweet_id))).execute(conn);
    match res {
        Ok(_) => Ok(()),
        Err(err) => Err(err),
    }
...
pub async fn list(path: Path<String>, pool: Data<DBPool>) -> HttpResponse {
    ...
    let likes =
        web::block(move || list_likes(Uuid::from_str(path.as_str()).unwrap(), &mut conn)).await.unwrap();
...
pub async fn plus_one(path: Path<String>, pool: Data<DBPool>) -> HttpResponse {
    ...
    let like =
        web::block(move || create_like(Uuid::from_str(path.as_str()).unwrap(), &mut conn)).await.unwrap();
...
pub async fn minus_one(path: Path<String>, pool: Data<DBPool>) -> HttpResponse {
    ...
    let _ = web::block(move || delete_like(Uuid::from_str(path.as_str()).unwrap(), &mut conn)).await;
